...
// Layout x-offsets for up to 3 banks in a single centered row (tuned per count)
const PROMO_ROW_LAYOUTS = {
  1: [0],
  2: [-160, 160],
  3: [-330, -30, 290],
};
...
function buildPromoSVG({ promoRate, promoBanks }){
  ...
  const positions = PROMO_ROW_LAYOUTS[banks.length] || PROMO_ROW_LAYOUTS[3];
  const bankRow = banks.map((b,i) => promoBankChip(positions[i] || 0, 0, b)).join('');
