...
function todayLabel(){
  const d = new Date();
  const dateStr = new Intl.DateTimeFormat('es-AR', { day: 'numeric', month: 'long', timeZone: 'America/Argentina/Buenos_Aires' }).format(d);
  const timeStr = new Intl.DateTimeFormat('es-AR', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'America/Argentina/Buenos_Aires' }).format(d);
  return `${dateStr} · ${timeStr} hs (ARG)`;
}
